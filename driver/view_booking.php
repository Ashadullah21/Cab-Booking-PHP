...
// Step 2: Validate the booking ID
if (!isset($_GET['id']) || empty($_GET['id'])) {
    die("Invalid booking ID passed. Please try again.");
}

$qry = $conn->query("SELECT b.*, c.contact as phone_number, c.firstname, c.lastname 
                     FROM `booking_list` b 
...
if ($qry->num_rows > 0) {
    ...
    die("No booking found for the given ID: " . $_GET['id']);
}
?>
        ...
        <div class="col-md-6">
            <fieldset class="bor">
                ...
                <dl>
                    ...
                    <dd class="pl-4"><?= isset($firstname, $lastname) ? $firstname . ' ' . $lastname : "N/A" ?></dd>
                    <dt class="">Client Phone Number</dt>
                    <dd class="pl-4"><?= isset($phone_number) ? $phone_number : "N/A" ?></dd>
                </dl>
            </fieldset>
        </div>
    </div>
...
<div class="text-right">
    <?php if(isset($status) && $status == 0): ?>
        <button class="btn btn-primary btn-flat bg-gradient-primary" type="button" id="confirm_booking">
            Confirm Booking
        </button>
        <button class="btn btn-danger btn-flat bg-gradient-danger" type="button" id="driver_cancel_booking">
            Cancel Booking
        </button>
    <?php elseif(isset($status) && $status == 1): ?>
        <button class="btn btn-warning btn-flat bg-gradient-warning" type="button" id="pickup_booking">
            Picked Up
        </button>
    <?php elseif(isset($status) && $status == 2): ?>
        <button class="btn btn-success btn-flat bg-gradient-success" type="button" id="dropoff_booking">
            Dropped Off
        </button>
    <?php endif; ?>
...
<script>
    $(function(){
        $('#confirm_booking').click(function(){
            _conf("Are you sure to confirm this booking [Ref. Code: <b><?= isset($ref_code) ? $ref_code : "" ?></b>]?", 
                  "update_booking_status", ["<?= isset($id) ? $id : "" ?>",1]);
        });

        $('#pickup_booking').click(function(){
            _conf("Mark [Ref. Code: <b><?= isset($ref_code) ? $ref_code : "" ?></b>] booking as Picked Up?", 
                  "update_booking_status", ["<?= isset($id) ? $id : "" ?>",2]);
        });

        $('#dropoff_booking').click(function(){
            _conf("Mark [Ref. Code: <b><?= isset($ref_code) ? $ref_code : "" ?></b>] booking as Dropped Off?", 
                  "update_booking_status", ["<?= isset($id) ? $id : "" ?>",3]);
        });

        $('#driver_cancel_booking').click(function(){
            _conf("Are you sure to cancel this booking [Ref. Code: <b><?= isset($ref_code) ? $ref_code : "" ?></b>]?", 
                  "driver_cancel_booking", ["<?= isset($id) ? $id : "" ?>"]);
        });
    });

    function driver_cancel_booking($id){
        start_loader();
        $.ajax({
            url: _base_url_ + "classes/Master.php?f=driver_cancel_booking",
            method: "POST",
            data: { id: $id },
            dataType: "json",
            error: err => {
